...
pub trait Lookup {
    ...
    fn lookup<T>(&self, id: IdT<T>) -> self::Result<&Item<T>>
    where
        T: Lookupable<Self::State>;

    fn lookup_mut<T>(&mut self, id: IdT<T>) -> self::Result<impl ops::DerefMut<Target = Item<T>>>
    where
        T: Lookupable<Self::State>;

    fn exists<T>(&self, id: IdT<T>) -> bool
    where
        T: Lookupable<Self::State>,
    {
