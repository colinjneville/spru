use std::fmt;

use crate::{action, item::{self, lookup}, PsuedoError, Record};


#[derive(Debug)]
pub struct Error {
    kind: Kind,
    context: Option<Context>,
}

impl Error {
    pub(crate) fn new(kind: Kind) -> Self {
        Self {
            kind,
            context: None,
        }
    }

    pub(crate) fn set_context<Action>(&mut self, record: &Record<Action>) {
        self.context = Some(Context::new(record));
    }
}

impl From<lookup::Error> for Error {
    fn from(value: lookup::Error) -> Self {
        Self::new(Kind::Lookup(value))
    }
}

impl From<item::id::Error> for Error {
    fn from(value: item::id::Error) -> Self {
        Self::new(Kind::Item(value))
    }
}

impl From<item::version::Error> for Error {
    fn from(value: item::version::Error) -> Self {
        Self::new(Kind::Version(value))
    }
}

impl From<action::Error> for Error {
    fn from(value: action::Error) -> Self {
        Self::new(Kind::Action(value))
    }
}

impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let Self {
            kind,
            context,
        } = self;

        write!(f, "Applying ")?;
        if let Some(context) = context {
            write!(f, "{context}")?;
        } else {
            write!(f, "Record")?;
        }
        write!(f, " failed: {kind}")?;

        Ok(())
    }
}

impl PsuedoError for Error {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match &self.kind {
            Kind::Lookup(e) => std::error::Error::source(e.as_error()),
            Kind::Item(e) => std::error::Error::source(e),
            Kind::Version(e) => std::error::Error::source(e),
            Kind::Action(e) => std::error::Error::source(e.as_error()),
        }
    }
}

#[derive(Debug)]
pub enum Kind {
    Lookup(lookup::Error),
    Item(item::id::Error),
    Version(item::version::Error),
    Action(action::Error),
}

impl fmt::Display for Kind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Kind::Lookup(e) => fmt::Display::fmt(e, f),
            Kind::Item(e) => fmt::Display::fmt(e, f),
            Kind::Version(e) => fmt::Display::fmt(e, f),
            Kind::Action(e) => fmt::Display::fmt(e, f),
        }
    }
}

#[derive(Debug)]
struct Context {
    index: usize,
}

impl Context {
    fn new<Action>(record: &Record<Action>) -> Self {
        Self {
            index: record.index,
        }
    }
}

impl fmt::Display for Context {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let Self {
            index,
        } = self;

        write!(f, "Record '{index}")?;

        Ok(())
    }
}