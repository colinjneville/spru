...
use crate::{common::{self, error::{FatalError, PsuedoError}}, game, interaction, interactor, item, player, Transaction};
...
impl<Client: self::Client> Messaging<Client> {
    ...
    pub fn push_signal<S: Into<common::signal::ToServerInternal<Client::Common>>>(&mut self, signal: S) {
        // TODO seq ids not yet implemented
        self.outbound.push(common::signal::ToServer { seq: 0, signal: signal.into() });
    }
...
pub trait Client: Sized {
    ...
    fn apply_interactions<Lookup>(&mut self, lookup: &mut Lookup, pending_interaction_id: Option<interaction::Pending>)
        -> Result<Output<Self, ()>, error::ApplyInteractionError>
    where 
        Lookup: item::Lookup<State = Self::State>,
    ;

    fn revert_interactions<Lookup>(&mut self, lookup: &mut Lookup, pending_interaction_id: Option<interaction::Pending>)
        -> Result<Output<Self, ()>, error::RevertInteractionError>
    where 
        Lookup: item::Lookup<State = Self::State>,
    ;

    fn signal<Lookup>(&mut self, lookup: &mut Lookup, arg: common::signal::ToClient<Self::Common>)
    ...
    Interaction: crate::Interaction<State = State, Action = Action, Root = Root>,
{
        ...
        Lookup: item::Lookup<State = Self::State>,
    {
        let inner = {
            ...
            snapshot.apply(lookup)
                .map_err(error_state.into_fatal())?;
        ...
        Lookup: item::Lookup<State = Self::State>,
    {
        ...
        let complete = interactor.complete(interaction_error)
            .map_err(|re| {
                if re.is_recovered() {
                    ...
                    error::StageInteractionError::Fatal(self.inner.error_state.into_fatal()(re.map_with(PsuedoError::into_error)))
                }
            })?;
        ...
        let pending_interaction_id = self.inner.log.stage_pending(interaction, expected_versions, Transaction::new(undo_records));
    ...
    fn apply_interactions<Lookup>(&mut self, _lookup: &mut Lookup, pending_interaction_id: Option<interaction::Pending>)
        -> Result<Output<Self, ()>, error::ApplyInteractionError>
    where
        Lookup: item::Lookup<State = Self::State>,
    {
        ...
        if !interactions.is_empty() {
            for interaction in interactions {
                state.push_signal(common::signal::ApplyInteraction {
                    interaction,
                });
            }
        }
        ...
        Lookup: item::Lookup<State = Self::State>,
    {
        ...
        self.inner.log.revert_pending(lookup, pending_interaction_id, false)
            .map_err(PsuedoError::into_error)
            .map_err(self.inner.error_state.into_fatal())?;
        ...
        Lookup: item::Lookup<State = Self::State>,
    {
        ...
        let ret = match signal {
            common::signal::ToClientInternal::InteractionResult(interaction_result) => self.interaction_result(&mut state, lookup, interaction_result)?,
            common::signal::ToClientInternal::ConfirmedTransaction(confirmed_transaction) => self.confirmed_transaction(&mut state, lookup, confirmed_transaction)?,
            common::signal::ToClientInternal::EndGame(end_game) => self.end_game(&mut state, lookup, end_game)?,
        };
...
{
    #[instrument(err, skip_all, fields(local_player_id = self.trace_id()))]
    fn interaction_result<Lookup>(&mut self, messaging: &mut self::Messaging<Self>, lookup: &mut Lookup, interaction_result: common::signal::InteractionResult<<Self as Client>::Common>)
        -> Result<(), FatalError>
    where
        Lookup: item::Lookup<State = State>,
    {
        ...
        if let Some((confirmed_transaction_id, extra_records)) = confirmed_transaction_id {
            self.inner.log.confirm_pending(lookup, pending_interaction_id, confirmed_transaction_id, &extra_records)
                .map_err(self.inner.error_state.into_fatal())?;
        } else {
            self.inner.log.revert_pending(lookup, None, true)
                .map_err(PsuedoError::into_error)
                .map_err(self.inner.error_state.into_fatal())?;
        }
    ...
    fn confirmed_transaction<Lookup>(&mut self, _messaging: &mut self::Messaging<Self>, lookup: &mut Lookup, confirmed_transaction: common::signal::ConfirmedTransaction<<Self as Client>::Common>)
        -> Result<(), FatalError>
    where
        Lookup: item::Lookup<State = State>,
    {
        ...
        self.inner.log.apply_confirmed(lookup, confirmed_transaction)
            .map_err(self.inner.error_state.into_fatal())?;
    ...
    #[instrument(err, skip_all, fields(local_player_id = self.trace_id()))]
    fn end_game<Lookup>(&mut self, messaging: &mut self::Messaging<Self>, lookup: &mut Lookup, end_game: common::signal::EndGame<<Self as Client>::Common>)
        -> Result<(), FatalError>
    where
        Lookup: item::Lookup<State = State>,
    {
        let common::signal::EndGame {
            game_outcome,
        } = end_game;
        // The game is ending anyway, so if we become desynced, just ignore it
        let _ = self.inner.log.revert_pending(lookup, None, true);

        messaging.events.push(event::GameComplete { game_outcome }.into());
