...
#[tagset(include(cloned::Actions<state_cell::StateCell<Option<crate::Play>>>))]
...
#[tagset(spru_util::maybe::Update<spru_util::fail::Update<pile::Pile<data::Card>>>)]
#[tagset(spru_util::fail::Update<pile::Pile<data::Card>>)]
pub struct Actions;
...
impl spru::action::Update for InitializeDeck {
    ...
    fn update(&self, value: &mut Self::T) -> AnyResult<Self::Undo> {
        ...
        let (new_pile, _) = pile::create(data::card::all().into_iter())
            .create()
            .expect("Infallible");
