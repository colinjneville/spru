...
impl<State: spru::State> spru::item::Lookup for Standalone<State> {
        ...
        T: item::lookup::Lookupable<Self::State>,
    {
        ...
        let item = if let Some(inner_map) = self.map.get(&type_id) {
            let inner_map = inner_map
                .downcast_ref::<HashMap<item::IdT<T>, Item<T>>>()
        ...
        T: item::lookup::Lookupable<Self::State>,
    {
        ...
        let inner_map = inner_map
            .downcast_mut::<InnerMap<T>>()
            .expect("Interal type mismatch");
        if let hash_map::Entry::Vacant(v) = inner_map.entry(id) {
            v.insert(value);
            Ok(())
        } else {
            Err(Error::IdAlreadyExists(id.untyped()).into())
        }
    }
